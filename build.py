...
def stack():
    w, h = 840, 424
    css = """
.spin{transform-origin:178px 232px;animation:spin 18s linear infinite}
.spin2{transform-origin:178px 232px;animation:spin 7s linear infinite reverse}
...
"""
    ...
    # ---- left: orbit + bus
    cx, cy = 178, 232
    b += f'<circle cx="{cx}" cy="{cy}" r="118" fill="{AMBER}" opacity=".05" filter="url(#soft)"/>'
    b += f'<circle cx="{cx}" cy="{cy}" r="72" fill="none" stroke="{FAINT}" stroke-width=".6"/>'
    ticks = ""
    import math
    for i in range(60):
        a = i * 6 * math.pi / 180
        ...
        ticks += (f'<line x1="{cx+r1*math.cos(a):.1f}" y1="{cy+r1*math.sin(a):.1f}" '
                  f'x2="{cx+r2*math.cos(a):.1f}" y2="{cy+r2*math.sin(a):.1f}" stroke="{FAINT}" stroke-width="1"/>')
    b += f'<g class="spin">{ticks}<circle cx="{cx}" cy="{cy}" r="104" fill="none" stroke="{AMBER}" stroke-opacity=".25" stroke-dasharray="2 7"/></g>'
    b += (f'<g class="spin2"><circle cx="{cx+104}" cy="{cy}" r="2.5" fill="{AMBER}" filter="url(#glow)"/></g>')
    hats = [("FRONTEND", -90, CYAN), ("DESIGN", -18, RED), ("DATABASE", 54, VIOLET),
            ("DEPLOY", 126, GREEN), ("BACKEND", 198, AMBER)]
    for name, deg, col in hats:
        a = math.radians(deg)
        dx, dy = cx + 104 * math.cos(a), cy + 104 * math.sin(a)
        lx, ly = cx + 121 * math.cos(a), cy + 121 * math.sin(a)
        anchor = "middle" if abs(math.cos(a)) < .3 else ("start" if math.cos(a) > 0 else "end")
        if deg == -90:
            ly -= 4
        b += f'<line x1="{dx:.1f}" y1="{dy:.1f}" x2="{cx+116*math.cos(a):.1f}" y2="{cy+116*math.sin(a):.1f}" stroke="{col}" stroke-opacity=".6"/>'
        b += f'<circle cx="{dx:.1f}" cy="{dy:.1f}" r="3" fill="{col}" filter="url(#glow)"/>'
        b += f'<text x="{lx:.1f}" y="{ly+3:.1f}" text-anchor="{anchor}" class="cap" font-size="8" fill="{col}">{name}</text>'
    b += pixel_bus(cx, cy)
    ...
    for label, sub, nw, boxed in nodes:
        if boxed:
            ...
            b += f'<rect x="{x}" y="134" width="{nw}" height="24" rx="4" fill="none" stroke="{FAINT}" stroke-dasharray="2 2"/>'
...
def footer():
    ...
    b += (f'<text x="{w/2}" y="28" text-anchor="middle" font-size="11.5" font-weight="700" fill="{TXT}">'
          f'the degree takes four years <tspan fill="{DIM}">—</tspan> <tspan fill="{AMBER}">the product gets better every time someone uses it</tspan></text>')
    b += f'<rect class="blink" x="752" y="18" width="6" height="12" fill="{AMBER}"/>'
    svg("footer", w, h, b, title="the degree takes four years — the product gets better every time someone uses it")


for f in (header, stack, loop, cells, meters, dots, footer):
    f()
